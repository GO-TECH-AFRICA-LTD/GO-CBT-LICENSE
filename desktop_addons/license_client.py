...
OFFLINE_OK = os.environ.get("GOCBT_OFFLINE_OK", "0") == "0"
...
def _default_state_path() -> str:
    try:
        if os.name == "nt":
            ...
            path = os.path.join(base, "GO_CBT", "license_state.json")
        else:
        ...
        return "license_state.json"

STATE_FILE = os.environ.get("GOCBT_STATE_FILE", _default_state_path())
...
def _state_path() -> str:
    # If STATE_FILE is absolute, use it as-is; otherwise resolve under the app dir
    try:
        return STATE_FILE if os.path.isabs(STATE_FILE) else os.path.join(_app_dir(), STATE_FILE)
    except Exception:
        return STATE_FILE
...
# Optional: block known dev/test credentials in production
if os.environ.get("GOCBT_BLOCK_TEST", "0") == "1":
    test_email = "gbengaoolabode@gmail.com"
    test_ref   = "T857124921543176"
    # Allow block by either email (user) or license_key match
    if (user or "").strip().lower() == test_email or (license_key or "").strip() == test_ref:
        return {"ok": False, "error": "Test credentials are disabled on this build."}
